$.fn.linkage = function(province, city, district, selectBox) {
    ...
    var cityUl = city + ' ul';
    var districtUl = district + ' ul';
    var pList = provinceList;
    var cList = provinceList[0].cityList;
    var dList = provinceList[0].cityList[0].areaList;
    // -------------------------------------------------------
    ...
    for (var a = 0; a < dList.length; a++) { //区循环列表
        var districtName = dList[a];
        var dLi = "<li>" + districtName + "</li>";
        $(district).find('ul').append(dLi);
    }
    /*-------------------------------默认城市结束--------------------------------------*/
    /*-------------------------------省市区三级联动开始--------------------------------------*/
    $(provinceUl).on('click', 'li', function(e) { //省级
            ...
            var indexDistrict = provinceList[pLiIndex].cityList[0].areaList;
            $(city).find('ul').html('');
            ...
            $(district).find('ul').html('');
            for (var j = 0; j < indexDistrict.length; j++) {
                var districtname = indexDistrict[j];
                var districtli = "<li>" + districtname + "</li>";
                $(district).find('ul').append(districtli);
            }
            $(cityInput).val(indexCity[0].name);
            $(districtInput).val(indexDistrict[0]);
    ...
    $(cityUl).on('click', 'li', function(e) { //市级
            ...
            if (districtLiIndex == "") {
                ...
                districtLiIndex = $('#pLiIndex').val();
            }
            $(city).find('input').val(cityText);
            var cityLiIndex = $(this).index();
            var indexArea = provinceList[districtLiIndex].cityList[cityLiIndex].areaList;
            $(district).find('ul').html('');
            for (j = 0; j < indexArea.length; j++) {
                var districtname = indexArea[j];
                var districtli = "<li>" + districtname + "</li>";
                $(district).find('ul').append(districtli);
            }
            $(districtInput).val(indexArea[0]);
            $(this).parent('ul').hide();
            e.stopPropagation();
        })
        /*------------------------------------------------------------------------------*/
    $(districtUl).on('click', 'li', function(e) { //区级
            var districtText = $(this).text();
            $(district).find('input').val(districtText);
            $(this).parent('ul').hide();
            e.stopPropagation();
        })
        /*-------------------------------省市区三级联动结束--------------------------------------*/
